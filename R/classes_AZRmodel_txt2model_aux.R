...
checkgetNotes <- function(textString) {
  ...
  notesk <- NULL
  if (startNotes[1] != -1) {
    notesk <- AZRaux:::strtrim(substr(textString,startNotes[1]+1,nchar(textString)))
    textString <- AZRaux:::strtrim(substr(textString,1,startNotes[1]-1))
...
checkGetFlag <- function(textString,flagString) {
  ...
  if (!is.null(AZRaux:::strlocateall(textString,flagString)$start)) {
    ...
    # set it to 0
    textString <- AZRaux:::strtrim(AZRaux:::strrep(textString,flagString,""))
    flagPresent <- TRUE
  }
...
checkGetSBMLinfo <- function(textString,fctname,elementname) {
  ...
  if (!is.null(infoStart$start)) {
    informationText <- AZRaux:::strtrim(substr(textString,infoStart$start+1,infoEnd$start-1))
    textString <- AZRaux:::strtrim(substr(textString,1,infoStart$start-1))
  }
  ...
  if(nchar(informationText)>0) {
    ...
    found <- FALSE
    if (tolower(AZRaux:::strtrim(terms[1]))=="isparameter") {
      type <- "isParameter"
      found <- TRUE
    }
    if (tolower(AZRaux:::strtrim(terms[1]))=="iscompartment") {
      type <- "isCompartment"
      if (length(terms)==1) terms[2] = ""
      compartment <- AZRaux:::strtrim(terms[2])
      found <- TRUE
    }
    if (tolower(AZRaux:::strtrim(terms[1]))=="isspecie") {
      type <- "isSpecie"
      if (length(terms)!=3)
        stop(paste(fctname, ": Error in ",elementname," isSpecie SBML information",sep=""))
      compartment <- AZRaux:::strtrim(terms[2])
      unittype <- AZRaux:::strtrim(terms[3])
      found <- TRUE
    }
    if (!found)
...
getParameters <- function(model,model_parameters) {
  ...
  if (!is.null(model_parameters)) {
    for (k in 1:length(model_parameters)) {
      parameterString <- AZRaux:::strtrim(model_parameters[k])
      ...
      test <- AZRaux:::strtrim(substr(parameterString,1,(temp[1]-1)))
      ...
      valuek = AZRaux:::strtrim(substr(parameterString,(temp+1),nchar(parameterString)))
...
getVariables <- function(model,model_variables) {
  ...
  if (!is.null(model_variables)) {
    for (k in 1:length(model_variables)) {
      variableString <- AZRaux:::strtrim(model_variables[k])
      ...
      test <- AZRaux:::strtrim(substr(variableString,1,(temp[1]-1)))
      ...
      formulak = AZRaux:::strtrim(substr(variableString,(temp+1),nchar(variableString)))
...
getFunctions <- function(model,model_functions) {
  ...
  if (!is.null(model_functions)) {
    ...
    for (k in 1:length(model_functions)) {
      ...
      functionString = AZRaux:::strtrim(model_functions[k])
      ...
      namek <- AZRaux:::strtrim(substr(functionString,1,(temp[1]-1)))
      ...
      test <- AZRaux:::strtrim(substr(functionString,(temp[1]+1),(temp2[1]-1)))
      ...
      formulak <- AZRaux:::strtrim(substr(functionString,(temp3[1]+1),nchar(functionString)))
...
getEvents <- function(model,model_events) {
  ...
  if (!is.null(model_events)) {
    for (k in 1:length(model_events)) {
      eventString <- AZRaux:::strtrim(model_events[k])
      ...
      namek <- AZRaux:::strtrim(substr(eventString,1,(temp[1]-1)))
      # check if event name given
      if (nchar(namek) == 0) {
        stop("getEvents: At least one event name not given.")
      }
      # get the right hand side
      eventRHS <- AZRaux:::strtrim(substr(eventString,temp[1]+1,nchar(eventString)))
