...
getOutputs <- function(model) {
  ...
  for (k in 1:Noutputs) {
    ...
    test <- cbind(AZRaux:::veclocate(outputFormula==states$statenames),AZRaux:::veclocate(outputFormula==variables$varnames))
    if (length(test)==0)
...
getFactorsTermsInput <- function(model,inputName,ODE) {
  ...
  if (ix$start==1) {
    ...
    ODEpre <- AZRaux:::strtrim(substr(ODE,1,ix$start-1))
  }
  ODEpost = AZRaux:::strtrim(substr(ODE,ix$end+1,nchar(ODE)))
...
getCheckNumberInputs <- function(model) {
  ...
  y      <- unique(unlist(y))
  y      <- sort(as.numeric(AZRaux:::strrep(y,"INPUT","")))
...
handleINPUTreactions <- function(model) {
  ...
  for (k in 1:length(ixReacInputs)) {
    ...
    for (k2 in 1:getNumberOfStatesAZRmodel(model)) {
      model$states[[k2]]$ODE <- AZRaux:::strrep(model$states[[k2]]$ODE,reacName,reacFormula)
    }
