...
SEXP cvodesAZRinterface(SEXP r_model_func_ptr,
  ...
  int     NREVENTS              = INTEGER(r_model_elements_nr)[4];
   ...
   int     NRTIMESTEPS           = LENGTH(r_timevector);
  ...
  for(int k=0; k<LENGTH(r_timevector); k++) timesimvector[k] = REAL(r_timevector)[k];
  for(int k=0; k<NRSTATES; k++) initialconditions[k] = REAL(r_ICs)[k];
  for(int k=0; k<NRPARAMETERS; k++) parametervector[k] = REAL(r_paramvalues)[k];
   if (verbose) {
    Rprintf("--------------------------------------\n");
  ...
  if (opt_xdotcalc) {
    ...
    PROTECT (output = allocVector(REALSXP, NRSTATES));
    for (int k=0; k<NRSTATES; k++) REAL(output)[k] = resultXdot[k];
    UNPROTECT(1);
    freeMem(0);
    return(output);
  }
  ...
  PROTECT (output = allocMatrix(REALSXP, NRTIMESTEPS, 1+NRSTATES+NRVARIABLES+NRREACTIONS));
  for (int krow=0; krow<NRTIMESTEPS; krow++) {
    REAL(output)[krow] = timesimvector[krow];
    for (int kcol=0; kcol<NRSTATES; kcol++) {
      REAL(output)[krow + NRTIMESTEPS*(kcol+1)] = statevalues[krow + NRTIMESTEPS*kcol];
    }
    for (int kcol=0; kcol<NRVARIABLES; kcol++) {
      REAL(output)[krow + NRTIMESTEPS*(kcol+NRSTATES+1)] = variablevalues[krow + NRTIMESTEPS*kcol];
    }
    for (int kcol=0; kcol<NRREACTIONS; kcol++) {
      REAL(output)[krow + NRTIMESTEPS*(kcol+NRSTATES+NRVARIABLES+1)] = reactionvalues[krow + NRTIMESTEPS*kcol];
    }
  }
  UNPROTECT(1);
  freeMem(1);
