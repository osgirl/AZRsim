/*
 * interpcse.c: Cubic spline interpolation with endpoints
 *
 * The syntax of this functions is as follows:
 *
 * yi = interpcse(x,y,xi)
 * yi = interpcse(x,y,xi,s1,s2)
 *
 * x:     x-values
 * y:     y-values
 * xi:    x-values at which to evaluate the spline function (allow multiple)
 * s1,s2: endpoint derivatives (if specified, both need to be given)
...
// Cubic spline interpolation with endpoints
//
// The syntax of this functions is as follows:
// yi = interpcse(x,y,xi)
// yi = interpcse(x,y,xi,s1,s2)
...
// @param x x-values
// @param y y-values
// @param xi x-values at which to evaluate the spline function (allow multiple)
// @param s1 first endpoint derivative
// @param s2 second endpoint derivative
// @return yi interpolated values
// [[Rcpp::export]]
NumericVector interpcse(NumericVector x, NumericVector y, NumericVector xi, double s1=NA_REAL, double s2=NA_REAL) {
  ...
  // Check x and y
  if (x.size() < 2) stop("interpcse: vector x should at least have a length of 2 elements");
  if (x.size() != y.size()) stop("interpcse: vectors x and y need to have same length");

  // Handle variable input arguments
  if (!internal::Rcpp_IsNA(s1) && internal::Rcpp_IsNA(s2)) stop("interpcse: if s1 is given then also s2 needs to be given");
  if (internal::Rcpp_IsNA(s1) && !internal::Rcpp_IsNA(s2)) stop("interpcse: if s2 is given then also s1 needs to be given");
  if (internal::Rcpp_IsNA(s1)) {
    // Neither s1 nor s2 are defined
    e1 = 0;
    e2 = 0;
    s1 = 0.0;
    s2 = 0.0;
  } else {
    // Both s1 and s2 are defined already
    e1 = 1;
    e2 = 1;
  }

  // Get number of knots and values to interpolate
  n = x.size();
  nx = xi.size();
  ...
  if (n==2) {
    /* If n==2 */
    if (e1 == 0 || e2 == 0) stop("interpcse: x and y of length n=2 requires the definition of slopes");
    b2 = s1;
    ...
    for (k=0;k<nx;k++) {
      yi[k] = y[0] + b2*(xi[k]-x[0]) + c2*pow(xi[k]-x[0],2.0) + d2*pow(xi[k]-x[0],3.0);
    }
  } else {
    /* If n>2 */
    ...
    spline(n, e1, e2, s1, s2, x.begin(), y.begin(), b, c, d, &flag);

    /* Call the spline evaluation function */
    for (k=0;k<nx;k++) {
      yi[k] = seval(n, xi[k], x.begin(), y.begin(), b, c, d, &last);
    }
